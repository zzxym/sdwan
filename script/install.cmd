...
function Get-BasicNetworkConfig {
    ...
    $ipChoice = Show-MultipleChoicePrompt -Message "请选择IP分配方式" `
        ...
        -Helps @("自定义此节点的IPv4地址，如果为空则仅转发数据包", "由sdwan自动确定并设置IP地址", "将仅转发数据包，不会创建TUN设备") `
        -DefaultIndex 1
    switch ($ipChoice) {
...
function Get-ExtraAdvancedOptions {
    ...
    if (Show-YesNoPrompt -Message "是否配置日志选项？" -DefaultIndex 1) {
        ...
        if (Show-YesNoPrompt -Message "是否指定日志目录？" -DefaultIndex 1) {
            $logDir = Get-InputWithDefault -Prompt "日志目录路径" -DefaultValue "$env:ProgramData\sdwan\logs"
    ...
    if (Show-YesNoPrompt -Message "是否配置其他高级选项？" -DefaultIndex 1) {
        ...
        foreach ($index in $selectedOther) {
            switch ($index) {
                ...
                5 { 
                    Write-Host "`n格式示例: wg://0.0.0.0:10010/10.14.14.0/24" -ForegroundColor DarkGray
...
$host.ui.rawui.WindowTitle = if ($Uninstall) { "卸载sdwan服务" } else { "安装sdwan服务" }  
Clear-Host
$ScriptRoot = (Get-Location).Path
$ServicesPath = Join-Path $ScriptRoot "services"
$RequiredFiles = @("sdwan-core.exe", "sdwan-cli.exe", "nssm.exe", "Packet.dll", "wintun.dll")
...
try {
    ...
    if ($Uninstall) {
        ...
        if (Show-YesNoPrompt -Message "确认安装配置？" -DefaultIndex 1) {
            & $nssm install $ServiceName (Join-Path $ScriptRoot "sdwan-core.exe")
            & $nssm set $ServiceName AppParameters $arguments
            & $nssm set $ServiceName Description "sdwan 核心服务"
