...
use sdwan::proto::common::Ipv4Inet;
use sdwan::print_completions;
use sdwan::{
    common::{
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: Ipv4Inet| ip.address.unwrap_or_default())
                        .map(|ip| ip.to_string())
                        .unwrap_or_default(),
            ...
            fn from(p: NodeInfo) -> Self {
                PeerTableItem {
                    ...
                    ipv4: Ipv4Inet::from_str(&p.ipv4_addr)
                        .map(|ip| ip.address.map(|addr| addr.to_string()).unwrap_or_default())
                        .unwrap_or_default(),
...
"#,
        ...
        SubCommand::GenAutocomplete { shell } => {
            ...
            print_completions(shell, &mut cmd, "easytier-cli");
        }
    }
